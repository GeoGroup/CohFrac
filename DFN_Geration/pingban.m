...
line_num = 100;
line_num_2 = 50;
set1 = Field(DFN('dim',2,'n',line_num,'dir',60,'ddir',-1e9,'minl',0.08,...
            'mu',0.9,'maxl',0.1,'dsep',0.001,'bbx',[0,0,1,1]),'Line');
set2 = Field(DFN('dim',2,'n',line_num_2,'dir',120,'ddir',-1e9,'minl',0.1,...
            'mu',0.2,'maxl',0.3,'dsep',0.001,'bbx',[0,0,1,1]),'Line');
...
write_dxf_head(fid1);
for i =1:size(set1,1)
    plot([set1(i,1),set1(i,3)],[set1(i,2),set1(i,4)],'LineWidth',2,'color','r');
    hold on
    line1(:,1)=set1(i,1);
    line1(:,2)=set1(i,2);
    line1(:,3)=0;
...
for i =1:size(set2,1)
    plot([set2(i,1),set2(i,3)],[set2(i,2),set2(i,4)],'LineWidth',2,'color','r');
    hold on
    line2(:,1)=set2(i,1);
    line2(:,2)=set2(i,2);
    line2(:,3)=0;
...
% end
% set1_len = set1_len';
% amplen_1=0*set1_len.*rand([set1_row,1]);
% amplen_2=0*set1_len.*rand([set1_row,1]);
...
% end
% set2_len = set2_len';
% amplen_3=0*set2_len.*rand([set2_row,1]);
% amplen_4=0*set2_len.*rand([set2_row,1]);
