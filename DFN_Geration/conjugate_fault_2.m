...
fid1=fopen('gonge_dn_03_2.dxf','w');
write_dxf_head(fid1);
for i =1:set2_row
...
%% 
figure(3)
for i = 1:size(line_group2,2)
   plot(line_group2{1,i}(:,1),line_group2{1,i}(:,2),'r');hold on
end
...
for i = 1:size(nset1,1)
    for j = 1:size(line_group2,2)
        for k = 1:size(line_group2{1,j},1)-1
            XY1 = [line_group2{1,j}(k,1);line_group2{1,j}(k,2)];
            XY2 = [line_group2{1,j}(k+1,1);line_group2{1,j}(k+1,2)];
        ...
        %     plot(out(1,:),out(2,:),'o','MarkerFaceColor','r');hold on
            ...
            if isempty(out2) == 0
                ...
                plot(trans(:,1),trans(:,2),'o','MarkerSize',4,'MarkerFaceColor','b','MarkerEdgeColor','b'); hold on
...
figure(5)
for i = 1:size(line_group2,2)
   plot(line_group2{1,i}(:,1),line_group2{1,i}(:,2),'LineWidth',2,'color','r');hold on
end
...
for i =1:set1_row
    ...
    start_point=[xpp(1),ypp(1)];
    end_point = [xpp(end),ypp(end)];
    for i = 1:size(xpp,2)
        p_dis1 = norm([xpp(i),ypp(i)]-start_point);
        p_dis2 = norm([xpp(i),ypp(i)]-end_point);
        if p_dis1 < 0.003
            xpp(i)=start_point(1);
            ypp(i)=start_point(2);
        end
